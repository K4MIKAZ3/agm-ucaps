...
export default async function DashboardPage() {
  ...
  if (authError) return dashboardError(`Sesión inválida: ${authError.message}`);
  ...
  try {
    ...
    try {
      const [kpiResult, proyectosResult, itemsResult, snapshotsResult] = await Promise.all([
        ...
        supabase
          ...
          .order("numero_item"),
        supabase
          .from("proyecto_snapshot_mensual")
          .select(
            `
            proyecto_id,
            avance_fisico_pct,
      ...
      loadError =
        error instanceof Error ? error.message : "No se pudo conectar con la base de datos.";
    }
      ...
      <main className="wrap">
        ...
        {!profile && !loadError && (
          <div className="alert-warn" style={{ marginBottom: 16 }}>
            Tu cuenta no tiene perfil en la base de datos.
          </div>
        )}
    ...
    const message =
      error instanceof Error ? error.message : "Error inesperado al renderizar el dashboard.";
    return dashboardError(message);
  }
}
